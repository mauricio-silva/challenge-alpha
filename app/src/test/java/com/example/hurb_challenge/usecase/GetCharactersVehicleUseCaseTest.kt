...
class GetCharactersVehicleUseCaseTest {

    private lateinit var repository: CharacterDetailsRepository
    private lateinit var useCase: GetCharacterVehiclesUseCase

    @Before
    fun setUp() {
        repository = mockk()
        useCase = GetCharacterVehiclesUseCase(repository)
    }

    @Test
    fun `should call getVehicles`() = runTest {
        val fakeUrls = listOf(
            "http://fakecharacterurl.com/01",
            "http://fakecharacterurl.com/02",
            "http://fakecharacterurl.com/03"
        )
        coEvery { repository.getVehicles(fakeUrls) } returns flowOf(Success(Mocks.listOfVehicle))
        useCase.call(fakeUrls)
        coVerify {
            repository.getVehicles(fakeUrls)
        }
    }
}